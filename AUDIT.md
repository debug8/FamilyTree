...
- ✅ **Фікс 2 — нерідні батьки/діти.** Закриті: D-C1, D-D11 (невідома стать у подружжі/свояцтві + невідома стать сполучної особи), D-D12 («зведений» → «неповнорідний»). Нове: `AffinityKind.StepParent/StepChild`, плечі `(1,0)` в `MapPatternA` і `(0,1)` в `MapPatternB`, назви вітчим/мачуха/пасинок/пасербиця та stepfather/stepmother/stepson/stepdaughter, тести `StepFamilyTests`.
- ✅ **Фікс 3 — подвійна перебудова дерева.** Закриті: A-C1, A-C3, A-B14, A-D23, A-D19 (частково — `TreeViewModel` тепер `IDisposable` з іменованими обробниками). Частково A-C2: прибрано зайві перебудови й доданий кеш бейджів, але сам `Compute` не пришвидшено (це D-B6, окремо).
- ✅ **Фікс 4 — крайові випадки родства.** Закриті: D-C2 (подружжя не губиться при кровному зв'язку — «дружина (також двоюрідна сестра)»), D-B3 (свояцтво враховує розлучення + чинний шлюб має пріоритет над розірваним), D-B4 (`SiblingKind.PossiblyHalf` — напіврідність не стверджується, коли другий батько невідомий). Нове: `KinshipContext.BloodRelationName`, `KinshipCalculator.BloodContext()`, `IsBetterAffinity()`, тести `KinshipEdgeCaseTests`. **Критичних більше не лишилось.**
- ✅ **Фікс 5 — надійність збереження.** Закриті: S-C1 (flush-to-disk), S-B5 (унікальний temp + `SemaphoreSlim`), S-B6 (`Move(overwrite)` + фолбек для `File.Replace`), S-B7 (короткі імена бекапів + `longPathAware` + провал бекапу некритичний), S-D12 (нумеровані слоти замість тиків із GUID), S-D13 (`SerializeAsync` у потік), S-D14 (`UpdatedAt` лише після успіху), S-D11 (кирилиця без escape-послідовностей). Нове: `WriteJsonAsync`, `Promote`, `RotateAndBackup`, `JsonFamilyStorage : IDisposable`, тести `SaveDurabilityTests`.
- ✅ **Фікс 6 — кілька шлюбів у розкладці.** Закриті: D-B5 (усі партнери на полотні + діти згруповані за своєю парою), D-D17 (`order.IndexOf` → мапа індексів, було O(n²)), D-D19 (детермінований порядок вузлів і ребер). Нове: `OrderChildrenByParentCouple`, тести `TreeLayoutMultipleMarriagesTests`. Рядок юніта тепер `S1 — Особа — S2`, тож обидві пари лишаються сусідніми.
- ⬜ Решта — нижче. **S-B8 (відновлення з резервної копії) лишається**: копії тепер мають передбачувані імена (`.1.bak` — найновіша) і відкриваються як звичайний документ, але UI-команди «Відновити з копії» ще немає.

**Скільки перерахунків дерева коштує дія (було → стало):**

| Дія | Було | Стало |
|-----|------|-------|
| Пошук / сортування зі збереженим виділенням | 2 | **0** |
| Повторний клік на тій самій особі | 1 | **0** |
| «Смуги поколінь», «Предки знизу» | 1 повний | **0** (лише перемалювання) |
| Зміна глибини/режиму | 1 повний | 1, але бейджі з кешу |
| Редагування / видалення / зміна зв'язку | 3 | **1** |
| Додавання особи | 3 | 2 (корінь дійсно змінюється) |

### Нові відкриті питання (виявлено під час виправлень)

- **Зведені сиблінги не визначаються.** Діти мачухи/вітчима — це шлях у **три** ребра (кров → шлюб → кров), а патерни A і B покривають лише два. Потрібен третій патерн. Зафіксовано тестом `Known_limitation_step_siblings_are_not_recognized_yet`.
- **Свояцтво не має ланцюжка у «Хто кому».** `KinshipPathExplainer.BuildChain` повертає `null` для будь-якого свояцтва (немає спільного предка), тож міні-граф порожній — тепер і для мачухи/пасинка. Не регресія (так було для тесть/зять), але для 2-хопових зв'язків ланцюжок `root → X → B` побудувати легко.
- **Step-grandparent не покрито.** Плечі `(2,0)` в патерні A (подружжя діда/баби) і `(0,2)` в патерні B (дитина онука подружжя) досі дають `NotAffinity`.
